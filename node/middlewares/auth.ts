const extractSessionData = async (ctx: Context) => {
  ...
  const adminUserId = sessionData?.namespaces?.authentication?.adminUserId?.value;
  const adminUserEmail = sessionData?.namespaces?.authentication?.adminUserEmail?.value;
