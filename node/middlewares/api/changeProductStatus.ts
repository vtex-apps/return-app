...
export async function changeProductStatus(ctx: Context, next: () => Promise<any>) {
    ...
    if (output.success) {
        ...
        if (requestResponse.length) {
            if (requestResponse[0].status === requestsStatuses.refunded) {
            ...
            } else if (requestResponse[0].status !== requestsStatuses.pendingVerification) {
                output = {
                    ...output,
                    success: false,
                ...
                if (productResponse.length) {
                    ...
                    if (providedQuantity > currentProduct.quantity) {
                        ...
                        const requestProductsResponse = await returnAppClient.getDocuments(ctx, 'returnProducts', 'product', `refundId=${request_id}`)
                        let refundedAmount = 0;
                        requestProductsResponse.map((currentProduct: any) => {
